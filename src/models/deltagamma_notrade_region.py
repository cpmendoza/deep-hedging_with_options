...
class DeepAgent(object):
    ...
    """
    def __init__(self, network, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, lambda_m, constraint_min, loss_type, lr, dropout_par, autoencoder, nbs_latent_spc, lambda_auto, name, process, initial_point):
        ...
        non_neg_constraint = StrictlyPositive(min_value=1e-5)#tf.keras.constraints.NonNeg()
        self.rebalancing_const_1   = tf.compat.v1.get_variable(name='rebalancing_const', shape=[1], initializer=tf.compat.v1.constant_initializer([initial_point]), trainable = True, constraint=non_neg_constraint)  # Add the constraint here
        ...
        for t in range(self.nbs_point_traj-1):
            ...
            if (t==0):
                ...
                for a in range(self.nbs_assets-1):
                    ...
                    #self.cost +=  tf.math.abs(self.hedging_instruments[t,:,a]*(self.deltas[t,:,a+1]-self.deltas[t-1,:,a+1]))*self.transaction_cost[1]
            ...
            if (t+1)<(self.nbs_point_traj-1):
                self.condition += tf.cast(tf.math.greater(self.input[t+1,:,2]-V_t, condition*self.constraint_min), tf.float32)
...
def train_network(network, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, lambda_m, constraint_min,
                   loss_type, lr, dropout_par, train_input, underlying_train, underlying_test, HO_train, HO_test,
                     cash_flows_train, cash_flows_test, risk_free_factor,dividendyield_factor, transaction_cost,
                       riskaversion, test_input, epochs, display_plot, id, autoencoder, nbs_latent_spc, lambda_auto, rebalancing_const, initial_point):
    ...
    try:
        ...
        if loss_type == "CVaR":
            name = f"Optimizer_delta_gamma_{network}_{auto_name}_dropout_{str(int(dropout_par*100))}_{loss_type}_{str(int(riskaversion*100))}_{transaction_cost_name}_{rebalance_name}_{id}_softcons_{lambda_m}"
        else:
            name = f"Optimizer_delta_gamma_{network}_{auto_name}_dropout_{str(int(dropout_par*100))}_{loss_type}_{transaction_cost_name}_{rebalance_name}_{id}_softcons_{lambda_m}"

        
        # Compile the neural network
        process = 'training'
        rl_network = DeepAgent(network, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, lambda_m, constraint_min, loss_type, lr, dropout_par, autoencoder, nbs_latent_spc, lambda_auto, name, process, initial_point)
